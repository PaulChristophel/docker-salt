...
# --- BEGIN: salt-token helpers (direct cache validation) ---

def _loadauth_from_opts():
    """
    Return a Salt auth object using the master opts that rest_cherrypy already
    has in cherrypy.config["saltopts"].
    """
    opts = cherrypy.config.get("saltopts") or {}
    try:
        return salt.auth.LoadAuth(opts)
    except Exception:  # pragma: no cover - version differences
        return salt.auth.Resolver(opts)
...
def _is_salt_token_valid(token):
    """
...
def salt_auth_tool():
    """
    Authenticate by validating a Salt token from header/cookie directly
    ...
    # Mirror into session if CherryPy sessions are enabled (compat)
...
class Logout(LowDataAdapter):
    ...
    def POST(self):  # pylint: disable=arguments-differ
        ...
        if token:
            lauth = _loadauth_from_opts()
            for name in ("rm_token", "del_token", "rm_tok"):
                fn = getattr(lauth, name, None)
                if callable(fn):
                    try:
                        fn(token)
                        break
...
class Events:
    ...
    def _is_valid_token(self, auth_token):
        ...
        # First, try to treat it as a rest_cherrypy session id (legacy)
        salt_token = None
        try:
        ...
        # If not found, treat the provided value as a raw Salt token
        candidate = salt_token or auth_token

        ok, _tok = _is_salt_token_valid(candidate)
        return bool(ok)
